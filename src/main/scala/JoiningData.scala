...
object JoiningData {
   ...
   *
   * @param dataDF The original DataFrame to perform the join operations on.
   ...
   */
  def run(dataDF: DataFrame, filteredCoaDF: DataFrame): DataFrame = {

    // Define the join condition: start date must be less than end date
    val onCondition = expr("start.date < end.date")

    // Alias the dataDF for the start and end DataFrames
    val startDF = dataDF.alias("start")
    val endDF = dataDF.alias("end")
    ...
    val onePartitionDF = dataDF.coalesce(1)
    ...
    val multiJoinDF = indexes.foldLeft(firstDF) { (currentDF, index) =>
      ...
      val nextDF = printNumParts(
        ...
        currentDF.join(onePartitionDF,
                       currentDF(s"month$prevIndex") <= dataDF("month"))
                 .select(currentDF("*"), dataDF("month").as(s"month$index"))
      )
